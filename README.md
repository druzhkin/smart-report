...
Перед `git push` прогнать import-smoke на основных модулях пайплайна:

```bash
python scripts/check_imports.py
```

Падает с ненулевым статусом, если какой-то импорт не разрешается — защита
от `git add`-недосмотра (см. `memory/untracked_pipeline_files_near_miss.md`).
